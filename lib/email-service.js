...
export class EmailService {
  ...
  static processTemplate(htmlContent, variables = {}) {
    ...
    const trackingPixel = `<img src="${process.env.SITE_URL || 'http://localhost:4321'}/api/email/track-open?campaign_id={{campaign_id}}&user_id={{user_id}}" width="1" height="1" style="display:none;" alt="">`;
        ...
        const emailPromises = batch.map(async (recipient) => {
          try {
            ...
            const variables = {
              ...
              unsubscribe_url: `${process.env.SITE_URL || 'http://localhost:4321'}/api/email/unsubscribe?user_id=${recipient.id}&campaign_id=${campaignId}`,
              campaign_id: campaignId,
              user_id: recipient.id
            };
            
            const processedContent = this.processTemplate(campaign.html_content, variables);
